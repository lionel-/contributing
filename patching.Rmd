...
Update the `trunk` branch to the latest sources with `git svn rebase`. Create a new branch for your changes and develop as you would normally with git commits.
...
- `make test-Specific`: Runs the main regression test files.

- `make check-devel`: Runs all tests minus those of the recommended package. This includes running examples of the base packages and checking the outputs conform and no unexpected errors arise.

- `make check-all`: Runs all tests including those of the recommended packages. You normally don't need to run this until you're ready to finalise a patch.
