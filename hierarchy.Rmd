...
- [`src/include/Defn.h`](https://github.com/wch/r-source/blob/trunk/src/include/Defn.h). Declarations for the private accessors, data structures, and functions.
...
- [`src/main/names.c`](https://github.com/wch/r-source/tree/trunk/src/main/names.c) defines the primitive function table (such as `list()`, `(`, or `eval()`). This table contains information about the primitive functions: a function pointer for the native routine, whether the primitive function is internal or exposed to the user, whether it evaluates its arguments and returns its value invisibly. For primitives implementing binary operators like `+`, the table also contains precedence information used for deparsing.
...
- The [`src/scripts`](https://github.com/wch/r-source/tree/trunk/src/scripts) folder contains shell scripts for `R CMD` commands. For instance, `R CMD check` is implemented in [`src/scripts`](https://github.com/wch/r-source/blob/trunk/src/scripts/check). The scripts are often very simple and invoke R code implemented in the tools package.
